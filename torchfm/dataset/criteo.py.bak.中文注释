    ...
    def __yield_buffer(self, path, feat_mapper, defaults, buffer_size=int(1e5)):
        ...
        with open(path) as f:
            ...
            for line in pbar:
                ...
                # 否则你拿另一份数据来预测是会有问题的。
                # 重点解析：之所以把稀疏特征和稠密特征都转换成索引，是因为第一索引是最简单的数据形式，不用管你每个特征具体的值是什么。第二
                # 相同特征值的索引是相同的，找不到的时候才用实际的值，值转换成索引是为了方便用词嵌入，这样转换成索引后就不再关心这个
                # 特征是稀疏还是稠密的了，都统一对待了，都放入词嵌入中进行统一编码，这样每个特征转换成[特征维度, 词嵌入的维度]的矩阵，
                # 每个特征作为DNN的输入层，经过N层隐藏层正向和随机梯度计算后，得到与实际label的误差loss。然后再经过深度学习框架反向
                # 计算梯度计算每一层的每个神经元的应该调整的权重，w=w-学习率*w，不断的学习直到整体样本误差最小或训练到指定epoch。
                """
