...
def train(model, optimizer, data_loader, criterion, device, log_interval=100):
    ...
    for i, (fields, target) in enumerate(tk0):
        ...
        # print('==={}, {}, {}'.format(type(fields), fields, target))
...
def main(dataset_name,
    ...
    for i in range(valid_dataset.__len__()):
        valid_dataset_labels.append(valid_dataset.__getitem__(i)[1])
    ...
    #valid_sampler = torch.utils.data.sampler.SubsetRandomSampler(np.random.choice(range(len(valid_dataset)), len(valid_dataset)))
    #test_sampler = torch.utils.data.sampler.SubsetRandomSampler(np.random.choice(range(len(test_dataset)), len(test_dataset)))


    train_data_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=0, sampler=sampler3(train_dataset_labels), shuffle=False)
    valid_data_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=0)#, sampler=sampler3(valid_dataset_labels), shuffle=False)
    test_data_loader = DataLoader(test_dataset, batch_size=batch_size, num_workers=0)#, sampler=sampler3(test_dataset_labels), shuffle=False)

    #train_data_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=0, sampler=SubsetRandomSampler(range(train_length)) )
    #valid_data_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=0)#, sampler=SubsetRandomSampler(range(train_length, valid_length)) )
    #test_data_loader = DataLoader(test_dataset, batch_size=batch_size, num_workers=0)

    #train_data_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=0, sampler=train_sampler)
    #valid_data_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=0, sampler=valid_sampler)
